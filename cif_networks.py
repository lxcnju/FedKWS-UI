...
class CifNet(nn.Module):
    def __init__(self, net, g_ratio=0.6, n_classes=35):
        super().__init__()
        self.encoder = load_encoder(
            net=net
        )
        self.n_classes = n_classes
        h_size = self.encoder.h_size
        g_size = int(2.0 * h_size * g_ratio)
        p_size = int(2.0 * h_size * (1.0 - g_ratio))

        self.glo_proj = nn.Linear(h_size, g_size, bias=False)
        self.pri_proj = nn.Linear(h_size, p_size, bias=False)

        self.glo_classifier = nn.Linear(g_size, n_classes)
        self.pri_classifier = nn.Linear(p_size, n_classes)

    def forward(self, x):
        out = self.encoder(x)
        glo_hs = self.glo_proj(out)
        pri_hs = self.pri_proj(out)
        glo_logits = self.glo_classifier(F.relu(glo_hs))
        pri_logits = self.pri_classifier(F.relu(pri_hs))
        return glo_hs, pri_hs, glo_logits, pri_logits


def load_encoder(net):
    if net == "ResNet":
        model = AudioResNetEncoder(
            n_layer=15,
            n_channel=45,
        )
    elif net == "DSCNN":
        model = AudioDSCNNEncoder(
            n_channel=172,
        )
    elif net == "MHAttRNN":
        model = AudioMHAttRNNEncoder(
            input_dim=40,
        )
...
class AudioDSCNNEncoder(nn.Module):
    def __init__(self, n_channel=172):
        super().__init__()
...
class AudioMHAttRNNEncoder(nn.Module):
    def __init__(self, input_dim=40, n_head=4, n_hidden=80):
        super().__init__()
...
class AudioResNetEncoder(nn.Module):
    def __init__(self, n_layer=15, n_channel=45):
        super().__init__()
